...
class PDF extends FPDF {
    ...
    $counts += mysqli_num_rows($querycount);
     ...
     $query = mysqli_query($conn, "SELECT * FROM reservation r join billing b on r.ReservationID=b.ReservationID where ReservationDate LIKE '%{$con}%' and Status LIKE '%{$status}%'") or die("error");
...
$pdf->Cell(130 ,5, ' ',0,0);
//$pdf->Cell(59 ,5,'Weekly'.' ReservationReport',0,1);//end of line
...
$pdf->Cell(300 ,5," for the " . $types . " of ". date("F, Y") ." : " . $counts,0,1);//end of line
//billing address
